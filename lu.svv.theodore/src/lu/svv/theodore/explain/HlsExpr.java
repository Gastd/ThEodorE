...
//import lu.svv.theodore.explain.SerialProxy;
//import lu.svv.theodore.explain.HlsExprFormatter;
//import lu.svv.theodore.explain.HlsStringTokenizer;

/**
 * Contains methods for parsing HLS expression.
 *
 * @author <a href="mailto:franz.wilhelmstoetter@gmail.com">Franz Wilhelmstotter</a>
 * @since 4.1
 * @version 7.1
 */
public final class HlsExpr
        implements Function<Double[], Double>, Serializable
{
         ...
         *
         * <pre>{@code
         * final TreeNode<Op<Double>> tree = HlsExpr.parseTree("1 + 2*(6 + 7)");
         * HlsExpr.CONST_REWRITER.rewrite(tree);
        ...
        // Primary constructor.
        private HlsExpr(final FlatTreeNode<Op<Double>> tree) {
        ...
        /**
         * Create a new {@code HlsExpr} object from the given operation tree.
         *
         ...
         */
        public HlsExpr(final Tree<? extends Op<Double>, ?> tree) {
         ...
         *
         * <pre>{@code
         *  final double result = HlsExpr.parse("2*z + 3*x - y").eval(3, 2, 1);
         *  assert result == 9.0;
         * }</pre>
         *
         * @see #apply(Double[])
        ...
        public boolean equals(final Object obj) {
                return obj == this ||
                        obj instanceof HlsExpr expr &&
                        _tree.equals(expr._tree);
        }

        /**
         * Return the string representation of this {@code HlsExpr} object. The
         ...
         *  final HlsExpr tree = HlsExpr.parse(expr);
         ...
         * @throws NullPointerException if the {@code rewriter} is {@code null}
         */
        public HlsExpr simplify(final TreeRewriter<Op<Double>> rewriter) {
        ...
        private Object writeReplace() {
                return new SerialProxy(SerialProxy.HLS_EXPR, this);
        }
        ...
        static HlsExpr read(final DataInput in) throws IOException {
         ...
         *  final String expr = "5.0 + 6.0*x + sin(x)^34.0 + (1.0 + sin(x*5.0)/4.0) + 6.5";
         *  final HlsExpr tree = HlsExpr.parse(expr);
         *  assert HlsExpr.format(tree.tree()).equals(expr);
         * }</pre>
         *
         * @since 4.3
         *
        ...
        public static String format(final Tree<? extends Op<Double>, ?> tree) {
                return HlsExprFormatter.format(tree);
        }
         ...
         *         can't be parsed.
         */
        public static HlsExpr parse(final String expression) {
                return new HlsExpr(FlatTreeNode.ofTree(parseTree(expression)));
        }
        ...
        public static Tree<Op<Double>, ?> parseTree(final String expression) {
                final var tokenizer = new HlsStringTokenizer(expression);
                return parseTree(tokenizer::next);
        }

        private static <V> Tree<Op<Double>, ?>
        parseTree(final Supplier<Token<String>> tokens) {
                final TreeNode<Op<Double>> tree = FORMULA_PARSER.parse(tokens, HlsExpr::toOp);
                Var.reindex(tree);
         ...
         *
         * <pre>{@code
         *  final double result = HlsExpr.eval("2*z + 3*x - y", 3, 2, 1);
         *  assert result == 9.0;
         * }</pre>
         *
         * @see #apply(Double[])
